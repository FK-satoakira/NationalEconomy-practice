...
//		this.cards.add(new C_P_HighSchool());
//		this.cards.add(new C_I_SteelMill());

		this.deck.getCards().add(new C_A_SlashAndBurn());
		for (int i = 0; i < 2; i++) {this.deck.getCards().add(new C_A_Farm());}
		for (int i = 0; i < 4; i++) {this.deck.getCards().add(new C_A_Orchard());}
		for (int i = 0; i < 2; i++) {this.deck.getCards().add(new C_A_LargeFarm());}

		for (int i = 0; i < 2; i++) {this.deck.getCards().add(new C_B_constructionCompany());}
		for (int i = 0; i < 2; i++) {this.deck.getCards().add(new C_B_GeneralContractor());}
		for (int i = 0; i < 2; i++) {this.deck.getCards().add(new C_B_ErhuConstruction());}

		this.deck.getCards().add(new C_F_CoffeeShop());
		for (int i = 0; i < 2; i++) {this.deck.getCards().add(new C_F_Restaurant());}

		for (int i = 0; i < 6; i++) {this.deck.getCards().add(new C_I_Factory());}
		for (int i = 0; i < 4; i++) {this.deck.getCards().add(new C_I_ChemicalFactory());}
		for (int i = 0; i < 4; i++) {this.deck.getCards().add(new C_I_SteelMill());}
		for (int i = 0; i < 4; i++) {this.deck.getCards().add(new C_I_CarFactory());}
		...
		this.deck.getCards().add(new C_Faci_LawOffice());
		for (int i = 0; i < 2; i++) {this.deck.getCards().add(new C_Faci_CompanyHousing());}
		for (int i = 0; i < 2; i++) {this.deck.getCards().add(new C_Faci_RealEstate());}
		for (int i = 0; i < 4; i++) {this.deck.getCards().add(new C_Faci_WareHouse());}
...
//						System.out.println(onroop);
					...
					case 1:
						if(player.getBoardOfPlayer().getCards().size()==0) {
							System.out.println("プレイヤーボードは空です");M.retry();continue;
						}
						int count = 0;
